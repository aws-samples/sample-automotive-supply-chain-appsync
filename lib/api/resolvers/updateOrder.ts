...
export function request(ctx: Context) {
  ...
  const { orderId, partId, orderDate, quantityOrdered, fulfilled } =
    ctx.args.input;
  ...
  if (partId !== undefined) updateValues.part_id = partId;
  if (orderDate !== undefined) updateValues.order_date = orderDate;
  if (quantityOrdered !== undefined)
    updateValues.quantity_ordered = quantityOrdered;
  if (fulfilled !== undefined) updateValues.fulfilled = fulfilled;
